...
public class MenuAssembler implements RepresentationModelAssembler<Menu, EntityModel<Menu>> {
    ...
    public EntityModel<Menu> toModel(Menu menu) {
        ...
        EntityModel<Menu> menuEntityModel = new EntityModel<>(menu,
                linkTo(methodOn(MenuController.class).getMenu(getMapper().getVendorId() , menu.getId())).withSelfRel(),
                linkTo(methodOn(MenuController.class).getAllMenus(getMapper().getVendorId(), pageable)).withRel("menus"));
